...
export default function EmailVerifiedPage() {
  ...
  useEffect(() => {
    ...
    //   • A URL hash fragment:  #access_token=...&type=signup   (implicit flow)
    //   • A query param:        ?code=...                        (PKCE flow)
    //   • An error param:       ?error=...&error_description=...
    ...
    // Success — either from hash type=signup or just landing here after verification
    if (hashType === "signup" || hashParams.get("access_token") || !errorParam) {
      setStatus("success");
    ...
    // Fallback — treat as success if we got here without an error
    setStatus("success");
    setMessage("Your email has been verified successfully!");
    const timer = setTimeout(() => {
      router.push("/login?verified=true");
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center bg-[#030303] overflow-hidden font-body text-white">
      ...
      <div className="relative z-10 w-full max-w-[420px] px-6 py-12 mx-auto text-center">
        <Link href="/" className="inline-block font-display text-[22px] text-white tracking-[0.2em] hover:text-[#c8a030] transition-colors duration-300 mb-10">
          LEESA MOHANTY
        </Link>
