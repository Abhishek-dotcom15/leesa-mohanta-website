...
export default function DomeGallery({
  ...
  const items = useMemo(() => buildItems(imagePool, segments), [imagePool, segments]);
  ...
  useEffect(() => {
    ...
    const speed = Math.max(0, Number(autoRotateSpeed) || 0);
    if (speed <= 0) return undefined;

    const tick = (timestamp) => {
      if (lastAutoFrameRef.current === 0) {
        lastAutoFrameRef.current = timestamp;
      }

      const deltaSeconds = Math.min(
        0.05,
        (timestamp - lastAutoFrameRef.current) / 1000
      );
      lastAutoFrameRef.current = timestamp;
      ...
      if (canAutoRotate) {
        const deltaY = autoDirectionRef.current * speed * deltaSeconds;
  ...
  useGesture(
    {
      ...
      onDrag: ({
        ...
        if (last) {
          ...
          let vx = vMagX * dirX;
          let vy = vMagY * dirY;
          const dragDx = Array.isArray(movement) ? movement[0] : 0;
          ...
          if (inferredDirection !== 0) {
            autoDirectionRef.current = inferredDirection;
          }
